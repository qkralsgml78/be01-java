package day18_jdbc_mvc.controller;
...
public class GiftController {  ///Controller : 모델과 뷰 단계에서 중계 역할을 해줌
	
	//연결, 삽입, 삭제, 수정, 검색 필요한 함수는 모두 다 여기에 넣을 것
	...
	private static void selectByGno(String className) throws SQLException {  // 
		...
		ResultSetMetaData rsmd = rs.getMetaData(); //interface
		int count = rsmd.getColumnCount();
		
		while( rs.next() ) {
			for (int i = 1; i <= count; i++) {  // 각 타입별로 출력하기
				//for 문에 1을 넣은 이유 : index는 1부터 시작
				
				switch (rsmd.getColumnType(i)) {
					case Types.NUMERIC ://Types. -->class이다.
					case Types.INTEGER ://Types이 int형이면 출력
						System.out.println(rsmd.getColumnName(i) + " : " +rs.getInt(i) + "  ");
						break;
					case Types.FLOAT ://타입이 float면
						System.out.println(rsmd.getColumnName(i) + " : " +rs.getFloat(i) + "  ");
						break;
					case Types.DOUBLE :// 타입이 double이면
						System.out.println(rsmd.getColumnName(i) + " : " +rs.getDouble(i) + "  ");
						break;
					case Types.CHAR :// 타입이 char이면
